...
<body class="bg-[url('resources/lib-bg.jpg')] font-nunito text-white flex">
    ...
    <main class="ml-25 m-5 p-15 w-[calc(100vw-135px)] min-h-[calc(100vh-40px)] h-auto rounded-4xl flex flex-col bg-[#eeeeee] z-2 text-dirty-brown drag-none">
        ...
        <div class="flex justify-between items-center">
            <div class="flex items-center gap-5">
                <span id="srch" class="relative">
                    ...
                    <input id="tsearch-box" placeholder="Search for a thesis book..." class="px-5 pl-8 py-1 w-80 border-2 rounded-2xl border-[#585345] text-[#585345] outline-none">
                </span>
                <select id="tsearch-category" class="px-5 py-1 w-50 h-9 border-2 rounded-2xl border-[#585345] text-[#585345] outline-none">
        ...
        <div id="all-ths" class="text-gray-100">
            <div class="mb-5  font-semibold text-[#585345] select-none flex items-center justify-between">
                <p class="text-2xl">Library</p>
                <span class="flex items-center justify-between gap-5">
                    <p class="text-sm italic">Showing <span id="tpage-info">1 out of 1</span> results.</p>
                    <select id="tsets-per-page" class="px-5 py-1 w-25 h-9 border-2 rounded-2xl border-[#585345] text-[#585345] outline-none">
            ...
            <div class="grid grid-cols-[repeat(auto-fit,_470px)] gap-y-12" id="theses-container">
    ...
    <script>
        ...
        function formatCourse(course) {
            return course == "BSIT-NS" ? "Bachelor of Science in Information Technology - Network Security" : "Bachelor of Science in Computer Science - Application Development";
        }

        let data = { theses: [] }, thesesPerPage = 24, selectedSet = 0, searchQuery = "", searchCategory = "";
        ...
        function displaySets() {
            ...
            tcontainer.innerHTML = thesesSlice.length ? thesesSlice.map(item => {
                let authorsList = item.authors.split("-"); // Split authors into an array
                let formattedAuthors = authorsList.map(a => "<li>" + (a == "et al." ? "et al." : upperWords(a)) + "</li>").join(""); // Format authors
                               
                return `
                <!-- (${item.thesis_id} MV) -->
                <div class="relative w-110 h-45 mini-view">
                    <div class="absolute w-full h-full flex">
                        ...
                        <div class="w-35 h-full select-none z-3">
                            <img src="resources/book.svg" alt="book" onclick="toggleFullView('${"fv" + item.thesis_id}')" class="absolute top-2.5 ml-2.5 w-30 hover:-translate-y-2 duration-200 cursor-zoom-in" draggable="false">
                        </div>
                        <div class="pt-8 w-70 flex flex-col gap-2 justify-start z-3">
                            <h2 class="leading-none text-sm text-justify">
                                ${upperWords(item.title)}
                            </h2>
                            <ul class="text-xs flex flex-wrap gap-1 *:px-3 *:rounded-xl *:bg-[#1b3a1a90] *:text-xs *:font-extralight *:opacity-85 *:select-all">
                                ${formattedAuthors}
                            </ul>
                            <p class="absolute bottom-2 right-3 text-xs opacity-80 select-none">${formatDate(item.published_date)}</p>
                        </div>
                    </div>
                </div>
                <!-- (${item.thesis_id} FV) -->
                <div class="fixed top-0 left-0 ml-25 m-5 w-[calc(100vw-135px)] min-h-[calc(100vh-40px)] h-auto rounded-4xl bg-[#eeeeee] z-5 text-black hidden" id="${"fv" + item.thesis_id }"> <!-- FULL VIEW -->
                    <div class="absolute top-1/2 right-0 -translate-y-1/2 p-15 w-full h-[60vh] grid grid-cols-[25%_75%] bg-[#355e3b]">
                        <div class="relative w-auto h-full">
                            <img src="resources/book.svg" alt="big-book" onclick="toggleFullView('${"fv" + item.thesis_id}')" class="absolute hover:-translate-y-4 duration-200 drop-shadow-2xl select-none cursor-zoom-out" draggable="false">
                        </div>
                        <div class="ml-15 flex flex-col text-justify text-gray-300">
                            <h2 class="text-xl leading-5 font-bold">${item.title}</h2>
                            <p class="mt-1 select-none opacity-80 font-semibold">${formatDate(item.published_date) + " | " + formatCourse(item.course)}</p>
                            <ul class="mt-2 flex flex-wrap gap-1.5 *:px-5 *:py-0.5 *:rounded-xl *:bg-[#1b3a1a90] *:text-sm select-none">
                                ${formattedAuthors}
                            </ul>
                            <hr class="mt-2.5 mb-5">
                            <p class="leading-5">${(item.abstract == "" ? "No abstract found." : item.abstract)}</p>
                            <br class="select-none">
                            <p><span class="font-bold"> ${item.keywords == "" ? "" : "Keywords: </span>" + item.keywords}</p>
                        </div>
                        <button class="absolute top-5 right-5 p-2 rounded-2xl bg-[#1b3a1a90] hover:opacity-80 active:scale-95 duration-100 cursor-pointer" onclick="toggleFullView('${"fv" + item.thesis_id}')">
                ...
                </div>`}).join("") : "<tr><td colspan='8' class='text-center'>No results found.</td></tr>";

                if (tpageInfo) tpageInfo.textContent = `${selectedSet + 1} of ${totalSets} set\/s`;
        }

        var currentId = "";
        var inFullView = false;

        document.addEventListener("keydown", e => {
            if (e.key === "Escape" && inFullView) {
                toggleFullView(currentId);
            }
        });
